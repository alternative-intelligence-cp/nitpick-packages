...
const char* nitpick_libc_net_resolve_all(const char* host) {
    if (strcmp(host, "localhost") == 0) return "127.0.0.1";
    return "";
}

const char* nitpick_libc_net_reverse_lookup(const char* ip) {
    if (strcmp(ip, "127.0.0.1") == 0) return "localhost";
    return "";
}

int64_t nitpick_libc_net_is_ipv4(const char* ip) {
