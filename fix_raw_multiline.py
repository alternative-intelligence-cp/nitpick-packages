...
for log in log_files:
    ...
    # The format is:
    # /path/to/file.npk:0:0: error: Line 22,
    #  Column 5: 'raw' argument must be Result<T> — got 'int32'.
    
    matches = re.finditer(r'(/home/randy/[^:]+\.npk):\d+:\d+: error: Line (\d+),\n[^\n]*\'raw\' argument must be Result<T>', content)
    for m in matches:
        filepath = m.group(1)
...
print("Done fixing multiline raw")
