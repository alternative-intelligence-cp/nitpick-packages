...
def fix_file(filepath):
    ...
    # Prepend raw to function calls
    for func in funcs_ret_res:
        # Match func(...) but not if preceded by raw or pub func: or func:
        # Need to handle assignments as well
        text = re.sub(r'(?<!raw )(?<!func:)(?<!pub func:)\b' + func + r'\(', r'raw ' + func + '(', text)

    with open(filepath, "w") as f:
        f.write(text)

fix_file("packages/nitpick-gradient-field/src/nitpick_gradient_field.npk")

# Apply the same to test, but wait, my fix_gf6.py already did it to test, but missed dg_decision_code?
# Actually, let's just re-generate test from original to be completely safe
...
text = re.sub(r'extern func:nitpick_alloc\s*=\s*wild float64\*\(\.\.\.\);\n', '', text)
text = re.sub(r'extern func:nitpick_free = NIL\(wild float64\*:ptr\);\n', '', text)
# Need to remove any other old externs:
text = re.sub(r'extern func:malloc\s*=\s*wild int64\*\(\w+:\w+\);\n', '', text)
text = re.sub(r'extern func:free\s*=\s*void\(wild int64\*:ptr\);\n', '', text)
...
text = re.sub(r'extern "libc" \{\s*func:nitpick_libc_exit = NIL\(int32:code\);\s*\}', 'extern "libc" { func:nitpick_libc_exit = NIL(int32:code); }', text)
...
text = re.sub(r'nitpick_free\((em_meta|gf_meta)\);', r'nitpick_free_i64(\1);', text)
text = re.sub(r'nitpick_free\((.*?)\);', r'nitpick_free_f64(\1);', text)
text = re.sub(r'free\((em_meta|gf_meta)\);', r'nitpick_free_i64(\1);', text)
text = re.sub(r'free\((.*?)\);', r'nitpick_free_i64(\1);', text)
